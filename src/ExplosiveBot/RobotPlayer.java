...
public strictfp class RobotPlayer {
    ...
    static void buildTraps() throws GameActionException {
        ...
        for(MapInfo m : rc.senseNearbyMapInfos(2)) {
            if(m.isSpawnZone() && m.getSpawnZoneTeamObject() != rc.getTeam()) {
                ok = true;
            }
        }
        if(!ok) return;
